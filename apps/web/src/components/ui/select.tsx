...
function shouldUseNativeSelect(props: NativeSelectProps | RadixSelectProps) {
...
function Select(props: NativeSelectProps | RadixSelectProps) {
  if (shouldUseNativeSelect(props)) {
    const { className, children, native: _native, ...nativeProps } =
      props as NativeSelectProps;

    return (
        <select
          className={cn(
          "h-11 w-full rounded-2xl border border-border bg-card/90 px-4 text-sm text-foreground outline-none transition focus:border-accent focus:ring-2 focus:ring-accent/15",
          className,
        )}
        {...nativeProps}
      >
  ...
  const { native: _native, ...radixProps } = props as RadixSelectProps;
  return <SelectPrimitive.Root data-slot="select" {...radixProps} />;
}
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          "relative z-50 max-h-[var(--radix-select-content-available-height)] min-w-36 overflow-x-hidden overflow-y-auto rounded-lg bg-popover text-popover-foreground shadow-md ring-1 ring-foreground/10 duration-100 data-[align-trigger=true]:animate-none data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95 [transform-origin:var(--radix-select-content-transform-origin)]",
          position === "popper"
